...
public sealed class PrintKitchenTicketCommandHandler
{
    ...
    public async Task<PrintResult> Handle(
        PrintKitchenTicketCommand command,
        CancellationToken cancellationToken = default)
    {
        var startedAt = _clock.UtcNow;

        var data = await _dataProvider.GetKitchenReceiptDataAsync(
            command.TicketNumber, cancellationToken);
        ...
        if (data is null)
        {
            ...
            {
                Success = false,
                ErrorMessage = "Kitchen ticket not found",
                StartedAt = startedAt,
        ...
        var document = _formatter.FormatKitchenTicket(data);
