...
$cat1 = curl.exe -s -X POST "$api/categories" -H "Content-Type: application/json" -d '"Main Course"' | ConvertFrom-Json
$catM = $cat1.id
Write-Host "  Main Course: $catM" -ForegroundColor Green

$cat2 = curl.exe -s -X POST "$api/categories" -H "Content-Type: application/json" -d '"Drinks"' | ConvertFrom-Json
$catD = $cat2.id
...
curl.exe -s -X POST "$api/products/$($p1.id)/suggested-prices" -H "Content-Type: application/json" -d "45" | Out-Null
...
curl.exe -s -X POST "$api/products/$($p2.id)/suggested-prices" -H "Content-Type: application/json" -d "55" | Out-Null
...
curl.exe -s -X POST "$api/products/$($p3.id)/suggested-prices" -H "Content-Type: application/json" -d "35" | Out-Null
...
curl.exe -s -X POST "$api/products/$($p4.id)/suggested-prices" -H "Content-Type: application/json" -d "10" | Out-Null
